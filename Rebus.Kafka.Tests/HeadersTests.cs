...
namespace Rebus.Kafka.Tests
{
    ...
    public partial class HeadersTests : BaseTestWithKafkaContainer
    {
        ...
        /// </summary>
        [Fact]//[Theory, MemberData(nameof(KafkaParameters))]
        public void MessageHeaderProduceConsume()
        {
            Output.WriteLine("start MessageHeaderProduceConsume");
            ...
            using (var producer = new ProducerBuilder<Null, string>(producerConfig).Build())
            {
                ...
                dr_duplicate = producer.ProduceAsync(singlePartitionTopic,
                    new Message<Null, string> { Value = "the value", Headers = headers3 }).Result;
                ...
                dr_ol1 = producer
                    .ProduceAsync(singlePartitionTopic, new Message<Null, string> { Value = "the value" }).Result;
                ...
                Action<DeliveryReport<Null, string>> dh = (dr) => drs.Add(dr);
            ...
            using (var producer = new ProducerBuilder<byte[], byte[]>(producerConfig).Build())
            {
                ...
                dr_ol4 = producer.ProduceAsync(singlePartitionTopic, new Message<byte[], byte[]> { Headers = null })
                    .Result;
                Assert.Empty(dr_ol4.Message.Headers);
                dr_ol5 = producer.ProduceAsync(singlePartitionTopic, new Message<byte[], byte[]> { Headers = null })
                    .Result;
                Assert.Empty(dr_ol5.Message.Headers);
                dr_ol6 = producer
                    .ProduceAsync(singlePartitionTopic, new Message<byte[], byte[]> { Headers = headers }).Result;
                ...
                Action<DeliveryReport<byte[], byte[]>> dh = (dr) => drs_2.Add(dr);
            ...
            using (var producer = new ProducerBuilder<Null, string>(producerConfig).Build())
            {
                ...
                nulldr = producer.ProduceAsync(singlePartitionTopic,
                    new Message<Null, string> { Value = "test-value", Headers = headers }).Result;
            ...
            Output.WriteLine("end   MessageHeaderProduceConsume");
        }

        private string singlePartitionTopic = "test-headers";
        ...
        public static IEnumerable<object[]> KafkaParameters()
        {
            ...
            {
                var assemblyPath = typeof(HeadersTests).GetTypeInfo().Assembly.Location;
